...
Based on the frequency choice discussed in section~\ref{sec:PoliticalandEconomicalLimitations} it is clear that the optimal frequency band is the $606~-~614~MHz$ band; a number of parameters are estimated and illustrated in table~\ref{tab:FrequencyDetails}.
...
% The pulse width is related to the bandwidth by the equation $\tau = 1/ B$.
...
The most important design requirement of the antenna is that is should have sufficient gain, it should be able to direct its energy in a specific direction. The second component of the antenna is that it should be capable of sending out this EM wave and receive the echo with minimal losses throughout. %This implies that it is designed to be as efficient as possible.
...
This principle is used to illustrate the scanning feature represented in Fig.~\ref{fig:SteeringSmall} where different scanning angles are illustrated by the differing curves. This scenario makes use of a $32 \times 32$ array where the steering is increased by $10^{\circ}$ increments. This is then scaled up to the standard sized array used for the system, this is illustrated in Fig.~\ref{fig:SteeringBig}.
...
The LNA is placed close to the physical antenna as this lowers the noise introduced via the transmission medium.% Efficient placement of these components components is required for this reduction of interference.
\begin{center}
    \begin{figure}
        \centering
